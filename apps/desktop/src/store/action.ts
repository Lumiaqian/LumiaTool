...
import { instanceOfInput, instanceOfHistorySerializable } from "@/lib/util";
import { paste, copy as copyText, copyImage } from "@/lib/clipboard";
...
import { objectInObject } from "@/lib/util";
...
export const initialize = async <T extends Record<string, any>>(
    ...
    const storeOperate = operateStore;
...
export const useAction = <T extends Record<string, any>>(input: InitializeReturn<T>) => {
    ...
    if (currentRef.current === null) {
        currentRef.current = proxy<T>(input.items);
    }
    const current = useMutable(currentRef.current);
    ...
    const historyPush = debounce(function() {
        ...
        try {
            history.push(handle(cloneDeep(snapshot(current))));
        } catch (e) {
        }
    }, 500);
