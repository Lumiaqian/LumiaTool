...
import { HttpError, postJson } from "@/lib/proxy";
...
export const getUsed = async () => {
    ...
    const response = await postJson<{ used: number }>("https://api.jdoodle.com/v1/credit-spent", {
        clientId: config.client_id,
        clientSecret: config.client_secret,
    });
    return Number(`${response.used}`);
};
export type Result = { output: string; memory: number; cpuTime: number; error: string };
export const execute = async (lang: string, code: string, version: string): Promise<Result> => {
    const config = getConfig();
    if (config.client_secret === "" || config.client_id === "") {
        throw new Error("not found config");
    }
    ...
    try {
        const response = await postJson<Result & { statusCode: number }>("https://api.jdoodle.com/v1/execute", {
            clientId: config.client_id,
            clientSecret: config.client_secret,
            script: code,
            language: lang,
        ...
        if (error instanceof HttpError) {
            const data = error.data as Partial<Result & { statusCode: number }> | null;
            if (data?.error) {
                throw new Error(`[${data.statusCode ?? error.status}]${data.error}\n${error.message}`);
            }
        }
        throw error;
    }
