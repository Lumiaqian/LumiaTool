...
import { Align, Button, Card, Input, InputNumber, Select } from "@/components";
...
const initial = await initialize({
    poor: { input1: current.format("YYYY-MM-DD HH:mm:ss"), input2: current.add(1, "d").format("YYYY-MM-DD HH:mm:ss"), unit: "seconds" },
...
export default function Calculator() {
    ...
    const poor = useMemo(() => dayjs(poorState.input2).diff(dayjs(poorState.input1), poorState.unit as QUnitType), [poorState.input1, poorState.input2, poorState.unit]);
    ...
    const operation = useMemo(() => {
        ...
        if (rate === 0) {
            return operationState.type === "+"
                ? dayjs(operationState.input).add(operationState.length, operationState.unit as ManipulateType).format("YYYY-MM-DD HH:mm:ss")
                : dayjs(operationState.input).subtract(operationState.length, operationState.unit as ManipulateType).format("YYYY-MM-DD HH:mm:ss");
        }
        return dayjs(dayjs(operationState.input).unix() * 1000 + rate * operationState.length * (operationState.type === "+" ? 1 : -1)).format("YYYY-MM-DD HH:mm:ss");
    ...
    const analyze = useMemo(() => {
        ...
        if (analyzeState.type === "quarter") {
            ...
            return $t("time_analyze_quarter_output", { quarter, weekOfQuarter: Math.ceil((input.unix() - quarterDate.unix()) / (86400 * 7)), dayOfQuarter: Math.ceil((input.unix() - quarterDate.unix()) / 86400), hourOfQuarter: Math.ceil((input.unix() - quarterDate.unix()) / 3600), minuteOfQuarter: Math.ceil((input.unix() - quarterDate.unix()) / 60), secondOfQuarter: input.unix() - quarterDate.unix() });
        }
        if (analyzeState.type === "month") {
            const month = input.month() + 1;
            const monthDate = dayjs(`${input.year()}-${month}-01`);
            return $t("time_analyze_month_output", { month, weekOfMonth: Math.ceil((input.unix() - monthDate.unix()) / (86400 * 7)), hourOfMonth: Math.ceil((input.unix() - monthDate.unix()) / 3600), minuteOfMonth: Math.ceil((input.unix() - monthDate.unix()) / 60), secondOfMonth: input.unix() - monthDate.unix() });
        }
        const yearDate = dayjs(`${input.year()}-01-01`);
        return $t("time_analyze_year_output", { year, quarter, weekOfYear: input.isoWeek(), dayOfYear: input.dayOfYear(), hourOfYear: Math.ceil((input.unix() - yearDate.unix()) / 3600), minuteOfYear: Math.ceil((input.unix() - yearDate.unix()) / 60), secondOfYear: input.unix() - yearDate.unix() });
    }, [analyzeState.input, analyzeState.type]);

    useEffect(() => {
        action.save();
    }, [action, poorState.input1, poorState.input2, poorState.unit, operationState.input, operationState.length, operationState.type, operationState.unit, analyzeState.input, analyzeState.type]);
        ...
        <Align direction="vertical">
            <Card title={$t("time_diff_tool")}>
                <Align>
                    ...
                    <span>相差</span>
                    <InputNumber center value={poor} width={160} append={<Select value={poorState.unit} onChange={(value: UnitValue) => { poorState.unit = value; }} options={unitOptions} />} />
                </Align>
            </Card>
            <Card title={$t("time_operation")}>
                <Align>
                    ...
                    <span>{$t("time_after")}, {$t("time_is")} <Button onClick={() => $copy(operation)} type="dotted" text={operation} /></span>
                </Align>
            </Card>
