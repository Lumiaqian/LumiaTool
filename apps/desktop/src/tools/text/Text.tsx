...
type TransformMethod = "upper" | "lower" | "upperLineStart" | "lowerLineStart" | "upperStart" | "lowerStart" | "replacePunctuation" | "zhTran" | "lineRemoveRepeat" | "rename" | "addLineIndex" | "removeLineIndex" | "sort" | "lineTrim" | "filterBlankLine" | "filterAllBr" | "escape" | "unescape" | "regularReplace" | "replace";
...
export default function Text() {
    ...
    const handle = useCallback((method: TransformMethod, option: HandleOption = {}) => {
    ...
    const escapeOptions = useMemo<CheckboxOption>(() => Object.keys(escapeChars).map(item => ({
        value: item,
        label: `${$t(`text_escape_${item}`)}(${escapeChars[item].string})`,
    })), []);

    const selectReplaceExplain = useCallback((value: string) => {
        ...
        <>
            <Card className="ctool-page-option" style={{ marginBottom: 5 }}>
                <Align horizontal="center">
                    ...
                    <Dropdown size={size} placeholder={$t("text_simplified_traditional")} options={[{ value: "simplified", label: `${$t("text_simplified")} -> ${$t("text_traditional")}` }, { value: "traditional", label: `${$t("text_traditional")} -> ${$t("text_simplified")}` }]} onSelect={(type: string) => handle("zhTran", { type })} />
                    ...
                    <Button size={size} text={$t("text_line_remove_duplicate")} onClick={() => handle("lineRemoveRepeat")} />
                    <Dropdown size={size} onSelect={(type: string) => handle("rename", { type })} placeholder={$t("text_rename")} options={renameTypeLists.filter(item => !["spaceCase", "pascalCaseSpace"].includes(item.value))} />
                    <Dropdown size={size} placeholder={$t("text_line_number")} options={[{ value: "addLineIndex", label: $t("text_line_number_add") }, { value: "removeLineIndex", label: $t("text_line_number_remove") }]} onSelect={(value: TransformMethod) => handle(value)} />
                    <Dropdown size={size} placeholder={$t("text_sort")} options={[{ value: "line_asc", label: $t("text_line_sort_asc") }, { value: "line_desc", label: $t("text_line_sort_desc") }, { value: "reverse_line", label: $t("text_reverse_line") }, { value: "reverse_line_string", label: $t("text_reverse_line_string") }, { value: "reverse_all", label: $t("text_reverse_all") }]} onSelect={(type: string) => handle("sort", { type })} />
            ...
            <Modal value={statMore} onChange={setStatMore} width={600} padding="0">
                <Tabs value="stat" lists={[{ label: $t("text_stat"), name: "stat" }, { label: $t("text_stat_explain"), name: "explain" }]} padding="0">
                    <Table columns={[{ title: $t("text_item"), key: "name1", width: 170 }, { title: $t("text_value"), key: "value1" }, { title: $t("text_item"), key: "name2", width: 170 }, { title: $t("text_value"), key: "value2" }]} lists={[{ name1: $t("text_string_length"), value1: stat.string_length, name2: $t("text_byte_length"), value2: `${stat.byte_utf8_length} / ${stat.byte_gbk_length}` }, { name1: $t("text_word_length"), value1: stat.word_length, name2: $t("text_line_length"), value2: stat.line_length }, { name1: $t("text_zh_length"), value1: `${stat.zh_word} / ${stat.zh_punctuation}`, name2: $t("text_en_length"), value2: `${stat.en_string} / ${stat.en_word} / ${stat.en_punctuation}` }, { name1: $t("text_int_length"), value1: `${stat.int_string} / ${stat.int_word}`, name2: "-", value2: "-" }]} />
                    <Table columns={[{ title: $t("text_item"), key: "name", width: 120 }, { title: $t("text_explain"), key: "explain" }]} lists={[{ name: $t("text_explain_byte_length_utf8_name"), explain: $t("text_explain_byte_length_utf8_info") }, { name: $t("text_explain_byte_length_gbk_name"), explain: $t("text_explain_byte_length_gbk_info") }, { name: $t("text_explain_string_length_name"), explain: $t("text_explain_string_length_info") }, { name: $t("text_explain_word_length_name"), explain: $t("text_explain_word_length_info") }, { name: $t("text_explain_int_length_name"), explain: $t("text_explain_int_length_info") }, { name: $t("text_explain_int_word_length_name"), explain: $t("text_explain_int_word_length_info") }, { name: $t("text_explain_blank_line_length_name"), explain: $t("text_explain_blank_line_length_info") }]} />
                </Tabs>
            </Modal>
            <Modal value={escapeShow} onChange={setEscapeShow} width={600} title={$t("text_escape")} footer={<Align horizontal="center"><Button text={$t("text_escape_forward")} onClick={() => handle("escape", { lists: action.current.escapeChars })} /><Button text={$t("text_escape_reverse")} onClick={() => handle("unescape", { lists: action.current.escapeChars })} /></Align>}>
                <Align horizontal="center"><Checkbox value={action.current.escapeChars} onChange={(value: EscapeCharsType[]) => { action.current.escapeChars = value; }} options={escapeOptions} /></Align>
            </Modal>
        </>
    );
}
