...
export const rowStyle = (value = ""): import("react").CSSProperties => {
    const [columnsValue, gapValue = "5"] = value.split("|");
    const columns = columnsValue.split("-");
    if (columns.length < 2) {
        return {};
    }
    return {
        ...
        gridTemplateColumns: columns
            .map(item => (/^\d+$/.test(item) ? `minmax(0, ${item}fr)` : item))
            .join(" "),
        gap: `${gapValue}px`,
    };
};
